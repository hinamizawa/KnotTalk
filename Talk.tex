...
\begin{frame}
	...
	\begin{figure}
		\centering
		\includegraphics[width=10cm]{pic/rmseq.png}
	\end{figure}
	\p
	\begin{block}{Remark}
	...
	% \end{figure}
	\begin{itemize}
		\item \RIa[yscale=-1]\myarrow\RIb \ ; \quad \RIIc\myarrow\RIIb
...
\begin{frame}[c]
	...
	\begin{figure}[!htb]
		...
		\begin{minipage}{4cm}
			\centering
			\includegraphics[width=3cm]{pic/Ochiai_unknot.png}
			\caption{One of Ochiai's unknot}
		\end{minipage}
		\begin{minipage}{4cm}
			\centering
...
\begin{frame}
	\begin{block}{Property of Writhe}
		The writhe $w(D)$ does not change if $D$ is changed under Reidemeister moves of type II or III; the writhe $w(D)$ does change by $+1$ or $-1$ if $D$ is changed under a Reidemeister move of type I. And the writhe of a knot diagram does \emph{not} depend on the choice of orientation.
	\end{block}

	For example:
	\begin{align*}
		w(\raisebox{-15pt}{\includegraphics[width=1.8cm]{pic/holflp.png}})  & =+2 \\ w(\raisebox{-15pt}{\includegraphics[width=1.8cm]{pic/holfpr.png}})&=-2\\
...
\begin{frame}
	...
	\begin{align*}
		\left\langle \raisebox{-10pt}{\includegraphics[width=1.6cm]{pic/holflp.png}} \right\rangle & = A\left\langle \raisebox{-10pt}{\includegraphics[width=1.6cm]{pic/hopfunkot2.png}} \right\rangle +A^{-1} \left\langle \raisebox{-10pt}{\includegraphics[width=1.6cm]{pic/hopfunkot1.png}} \right\rangle \\
...
\begin{frame}
	...
	\begin{figure}
		...
		\caption{The left-handed and right-handed trefoil knots are not equivalent}
	\end{figure}
...
\begin{frame}
	...
	\begin{block}{Sketch of proof}
		Consider a calculation if the polynomial of $K_1+K_2$ and operate firstly on the crossings of just one summand.
	\end{block}
...
\begin{frame}
	...
	\begin{block}{Definiton-Theorem}
		The Jones polynomial invariant is a function\[V:\{\mbox{Oritented links in }S^3\}\to \ZZ[t^{1/2}, t^{-1/2}]\] such that \begin{enumerate}[I.]
			\item $V(\KPA)=1$
			\item whenever three oriented links $L_+, L_-$ and $L_0$ are the same, except in the neighbourhood of a point where they are shown as in the next slide, then 
...
\begin{frame}[c]
	...
	\end{center}
	\[t^{-1}V(L_+)- tV(L_-)+(t^{-1/2}-t^{1/2})V(L_0)=0\]
	But $L_+$ and $L_-$ are the unknot, so $V(L_+)=V(L_-)=1$.
	\begin{align*}
...
\begin{frame}
	...
	The conjecture has been confirmed for several families of knots, including alternating and adequate knots, knots up to 18 crossings.
\end{frame}
...
\begin{frame}
	\frametitle{Coloured Jones Polynomials}
		...
		None of the three methods discussed in this talk was Jones' original approach, whose original formulation of his polynomial came from his study of operator algebras. His work is generalized to vase families of invariants, called quantum invariants.

		\begin{itemize}
			\item The coloured Jones polynomials is a infinite sequence of Laurent polynomials $\{J_{K,n}(t)\}_n$, encoding the Jones polynomial of $K$ and these of the links $K^s$ that are the \emph{parallels} of $K$.
