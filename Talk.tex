...
\begin{frame}
	...
	\begin{block}{Unkotting Problem}
		Given a knot (diagrams), determining whether it is the unknot.
	\end{block}
	...
	{\small{\begin{itemize}
			...
			\item $c=$ the sum of crossing numbers of a knot diagram; an upper bound on the number on the number of Reidemeister moves required to arrive at the standard unknot is $(236c)^{11}$ (Lackenby 2015).
		\end{itemize}}}
\end{frame}
...
\begin{frame}
	...
	$\left\langle\KPUNLINK\right\rangle=(-A^{2}-A^{-2})\left\langle\KPA\right\rangle=-A^2-A^{-2}$
	\newline
	\vv
